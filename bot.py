...
async def change_status():
    ...
    while not bot.is_closed:
        ...
        await bot.change_presence(game=discord.Game(name=client_status))
...
async def help(ctx):
    ...
    embed.add_field(name="Links", value="Give you Nekashu links.", inline=True)
    embed.add_field(name="Suggest <What you would like to suggest>", value="Send a suggestion to what Nekashu should do.", inline=True)
    ...
    embed.add_field(name="Equip", value="States the equipment Nekashu uses.", inline=True)
        ...
        if word.lower() in chat_filter:
            if not message.author.id in bypass_filter:
                try:
                    ...
                    await bot.send_message(message.channel, ":negative_squared_cross_mark: **You can't say that, Sorry!**")
...
async def report(ctx,reported:discord.Member, *, message):
    ...
    await bot.send_message(bot.get_channel("512613874076352513"), message)
...
async def info(ctx, user: discord.Member):
    ...
    if ctx.message.author.server_permissions.kick_members:
        ...
        await bot.say(":negative_squared_cross_mark: **No Permission**")
...
async def server(ctx):
    if ctx.message.author.server_permissions.kick_members:    
        ...
        embed.set_author(name="Nekashu Public Discord")
...
@bot.command(pass_context=True)
async def links(ctx):
    await bot.say('**Youtube**\nhttps://www.derex.pl\n**Bot**/nhttps://discordapp.com/oauth2/authorize?&client_id=512611578407813131&scope=bot&permissions=8')
...
async def suggest(ctx,*,  bugs: str):
    ...
    await bot.send_message(bot.get_channel("512606829071695872"), embed=emb_3_1)
...
async def mute(ctx, user: discord.Member,*,  bugs: str):
     if ctx.message.author.server_permissions.administrator or ctx.message.author.id == '417445689249890304':
        ...
        await bot.send_message(bot.get_channel("512612990273716264"), embed=embed)
     else:
        embed=discord.Embed(title=":negative_squared_cross_mark: **No Permission**", description="You don't have permission to use this command.", color=0xff00f6)
        await bot.say(embed=embed)
#Ban----------#
@bot.command(pass_context=True)
async def ban(ctx, user: discord.Member):
    if ctx.message.author.server_permissions.kick_members:
       ...
       embed = discord.Embed(title=":negative_squared_cross_mark: **No Permission**", description="You don't have permission to use this command.", color=0xff0000)
...
async def kick(ctx, user: discord.Member):
    ...
    if ctx.message.author.server_permissions.kick_members:
       ...
       embed = discord.Embed(title=":negative_squared_cross_mark: **No Permission**", description="You don't have permission to use this command.", color=rant)
       embed.set_footer(text="Sorry.")
       await bot.say(embed=embed)       
...
async def warn(ctx, user: discord.Member,*,  bugs: str):
    ...
    await bot.send_message(bot.get_channel("512612990273716264"),embed=emb_3_1)
#-----------------TESTING------------------------------#
